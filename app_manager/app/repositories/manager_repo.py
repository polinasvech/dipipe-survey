...
from app_manager.app.models.manager_model import Manager as ManagerSchema, CreateManagerRequest
...
class ManagerRepo:
    ...
    def _map_to_model(self, manager: DBManager) -> ManagerSchema:
        return ManagerSchema(**dict(manager))

    def _map_to_schema(self, manager: ManagerSchema | CreateManagerRequest) -> DBManager:
        return DBManager(**manager.model_dump())

    def create_manager(self, manager: CreateManagerRequest) -> ManagerSchema:
        try:
    ...
    def get_all_managers(self) -> List[ManagerSchema]:
        return [self._map_to_model(m) for m in self.db.query(DBManager).all()]

    def get_manager_by_id(self, manager_id: UUID) -> ManagerSchema:
    ...
    def update_manager(self, manager: ManagerSchema) -> ManagerSchema:
        try:
